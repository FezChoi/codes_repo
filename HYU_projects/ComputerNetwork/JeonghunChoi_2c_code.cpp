...
int main(){
    ...
    for(int i = 0; i < EdgeVector.size(); i++){
        if(EdgeVector[i].src == 1 && EdgeVector[i].dst == 2){
            EdgeVector.erase(EdgeVector.begin()+i); // erase edge from A to B.
        }
        if(EdgeVector[i].src == 2 && EdgeVector[i].dst == 1){
            EdgeVector.erase(EdgeVector.begin()+i); // erase edge from B to A.
        }        
    }    
