...
aa= '20200731_5_41_7_30deg_30deg.bin';
...
arr_tmp_rmN = raw_br_phase_unw;
for ii =3:length(raw_br_phase_unw)
   arr_tmp_rmN(ii)  = filter_removeImpluseNoise(raw_br_phase_unw(ii-2),...
                                                                            raw_br_phase_unw(ii-1), raw_br_phase_unw(ii), 0.4 );
end

raw_br_phase_unw_dis = arr_tmp_rmN;%*para.lambda/(4*pi);
...
arr_tmp_len = d_length - 199;
arr_tmp = zeros(arr_tmp_len,1);
% arr_tmp2 = zeros(arr_tmp_len-4,1);

% select 200 frames, find index of max frequency.
for ii = 1:(d_length-199)
    ...
    [br_v, br_i] = max(tmp);    
    arr_tmp(ii,1) = br_i/256*600;
end

plt2.x = linspace(0,300,arr_tmp_len);
subplot(211);
plot(plt2.x, arr_tmp');axis([0 300 0 50]);
% hold on
% load('ECGdata_20200731_5_41_7');
% plot(1:300,ECGdata_20200731_5_41_7,'Color','g');
% hold off
% % plt2.x = linspace(0,300,length(arr_tmp2));
% % subplot(2,1,2);plot(plt2.x, arr_tmp2');axis([0 300 0 50]);

subplot(212);
arr_tmp_rmN = zeros(length(arr_tmp),1);

for ii = 200:(length(arr_tmp)-199)
   arr_tmp_rmN(ii)  = mean(arr_tmp(ii-199:ii+199,1));
end

%plot(plt2.x, arr_tmp');
plot(plt2.x, arr_tmp_rmN');axis([0 300 0 50]);
hold on

tmptmp = zeros(300,1);
for ii = 10:(300-9)
    tmptmp(ii) = mean(ECGdata_20200731_5_41_7(ii-9:ii+9,1));
end
plot(1:300,tmptmp','Color','g');
hold off


% for ii = 1:(length(raw_br_phase_unw)-300)
%     Margin = ii;
%     Margin_end = ii+300;
%     
% %     subplot(311);plot(Margin:Margin_end, raw_br_phase_unw(Margin:Margin_end,1)');axis([-inf inf -inf inf]);
%     tmp = abs(fft(raw_br_phase_unw(Margin:Margin_end,1)',512));
%     subplot(311);plot(plt.x,tmp(1:256));axis([-inf inf -inf inf]);
%     
% %     subplot(312);plot(Margin:Margin_end, raw_br_phase_rmN(Margin:Margin_end,1)');axis([-inf inf -inf inf]);
%     tmp = abs(fft(raw_br_phase_rmN(Margin:Margin_end,1)',512));
%     subplot(312);plot(plt.x,tmp(1:256));axis([-inf inf -inf inf]);    
%     
% %     subplot(313);plot(Margin:Margin_end, raw_br_phase_unw_iir(Margin:Margin_end,1)');axis([-inf inf -inf inf]);
%     tmp = abs(fft(raw_br_phase_unw_iir(Margin:Margin_end,1)',512));
%     subplot(313);plot(plt.x,tmp(1:256));axis([-inf inf -inf inf]);    
% 
%     pause(0.001);
% end

% for ii = 1:(length(raw_br_phase_unw)-200)
%     Margin = ii;
%     Margin_end = ii+200;
%     tmp = raw_br_phase_unw_iir_hr(Margin:Margin_end,1)';
%     subplot(211);plot(Margin:Margin_end, tmp);axis([-inf inf -inf inf]);
%     tmp = abs(fft(tmp,512));
%     subplot(212);plot(plt.x,tmp(1:256));axis([-inf inf -inf inf]);
%     pause(0.001);
% end