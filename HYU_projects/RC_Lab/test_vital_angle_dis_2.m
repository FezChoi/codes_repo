...
aa= '20200807_11_33_1_30deg_30deg.bin';
RawData = readDCA1000(aa,para.adcsample);
load('ECGdata_20200807_11_33_1_br');
ECGData_br = ECGdata_20200807_11_33_1_br;
load('ECGdata_20200807_11_33_1_hr');
ECGData_hr = ECGdata_20200807_11_33_1_hr;
...
arr_tmp = zeros(d_length,1);
arr_tmp2 = zeros(d_length,1);
plt2.x = linspace(0,300,d_length);

for ii = 1:(d_length-399)
    Margin = ii;
    Margin_end = ii+399;
    Margin_sec = round(5 + Margin_end/20,0);
    
    tmp  = raw_br_phase_unw_iir(Margin:Margin_end,1)';
    tmp2 = raw_br_phase_unw_iir_hr(Margin:Margin_end,1)';
    
    subplot(311);plot(Margin:Margin_end, tmp);
    hold on
    plot(Margin:Margin_end, tmp2);
    hold off
    axis([-inf inf -inf inf]);
    title(['respiration/heart graph (',num2str(Margin_end),'/',num2str(d_length),')']);
    
    tmp  = abs(fft(tmp,512));
    tmp2 = abs(fft(tmp2,512));
    [br_v, br_i] = max(tmp); rr = (br_i-1)*600/255;
    [hr_v, hr_i] = max(tmp2); hr = (hr_i-1)*600/255;
    
    subplot(312);plot(plt.x,normalize(tmp(1:256)));
    hold on
    plot(plt.x,normalize(tmp2(1:256)));
    hold off
    axis([-inf inf -inf inf]);
    title(['respiration = ', num2str(round(rr,2)), ', heart = ', num2str(round(hr,2))]);
    disp(['scanning ', num2str(Margin), ' : ', num2str(Margin_end), ...
        ', resp = ', num2str(round(rr,2)), ', heart = ', num2str(round(hr,2))]);
    
    arr_tmp(Margin_end,1) = rr;
    arr_tmp2(Margin_end,1) = hr;
    
    subplot(313);plot(plt2.x, arr_tmp');
    hold on
    plot(plt2.x, arr_tmp2');
    axis([0 300 0 140]);
    plot(1:300,ECGData_br,'Color','g');
    plot(1:300,ECGData_hr,'Color','m');
    hold off
    title(['respiration trend (',num2str(Margin_sec),'sec)']);
...
arr_ecg_br_mean = zeros(length(arr_tmp),1);
arr_ecg_hr_mean = zeros(length(arr_tmp),1);
for ii = 20:(length(ECGData_br)-19)
    arr_ecg_br_mean(ii) = mean(ECGData_br(ii-9:ii+9));
    arr_ecg_hr_mean(ii) = mean(ECGData_hr(ii-9:ii+9));
end

plot(plt2.x, arr_tmp');
hold on
plot(plt2.x, arr_tmp_mean');
plot(plt2.x, arr_ecg_br_mean','Color','g');
plot(plt2.x, arr_ecg_hr_mean','Color','m');
axis([0 300 0 50]);
hold off
